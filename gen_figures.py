...
BASE_STYLE = (
 ...
 "handwriting-style text for ALL labels (like the Virgil / hand-drawn font). Use a strict semantic "
 "COLOR-AS-INK palette (color carries meaning): BLUE ink for mechanisms and how data moves, GREEN ink "
 "for sizes / counts / specific numeric values, RED ink for labels and warnings, PURPLE ink for code "
 "snippets and identifiers, ORANGE ink for emphasis callouts, pale-yellow diagonal hatching for state / "
 "containers / buffers. Long thin slightly-curved dashed arrows connect each margin note to the exact "
 "component it describes. Hand-drawn numbered circles (1)(2)(3) marking reading order. Key takeaways go "
 "inside a dashed rounded box. "
 "IMPORTANT FIDELITY RULES: Render ONLY what the DIAGRAM TO DRAW describes. Do NOT invent numbers, "
 "constants, limits, timeouts, config values, or a 'CONSTANTS' / parameters box — include specific values "
 "ONLY if the description states them explicitly. Never write the name of a color (e.g. 'blue') or the "
 "word 'annotation' as visible text; color is ink only, not a label. "
 "Flat, no shadows, no gradients, no photorealism, no typeset fonts, wide composition, generous white "
 "space, clean and legible handwriting."
)
